...
AI-OpsLog 是一个运维日志分析助手 Demo 项目。当前版本实现 FastAPI 后端服务：规则解析日志、生成 Markdown 报告，并可调用阿里云百炼 / 通义千问 DashScope OpenAI 兼容接口输出结构化故障分析 JSON。
...
返回中的 `markdown_report` 字段是完整 Markdown 报告，`report_path` 是保存后的文件路径。
